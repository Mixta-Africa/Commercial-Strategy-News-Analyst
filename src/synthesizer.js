...
Respond ONLY with a valid JSON block matching this structural layout exactly (no markdown formatting, no preambles)[cite: 2]:
{
  ...
  /**
   * Update theme memory: calendar-aware temporal tracking.
   * Increments count ONLY if the theme is seen on a new, unique calendar date.
   * Prevents artificial inflation from multiple manual executions on the same day.
   */
  updateMemory(memory, briefing, dateStr) {
    ...
    for (const theme of briefing.themes) {
      ...
      if (prior) {
        // STRICT CALENDAR LOCK: Only increment the counter if it is a genuinely new day
        if (prior.lastSeen !== dateStr) {
          prior.count += 1;
          prior.lastSeen = dateStr;
        }
        // Always keep the latest, sharpest phrasing generated by the LLM
        prior.label = label; 
      } else {
    ...
    // Roll the window: keep the most recently seen 60 themes to prevent bloat
